...
Anima 实验档需要额外记录：`anima_base` 的 LLLite 线稿条件是否生效、`anima_2_9b` 的低降噪输入尺寸、工作流直出尺寸、模型加载耗时、热推理耗时、显存峰值和颜色覆盖。两个档位均不调用页面 VLM、不使用角色参考图、不走 Real-CUGAN 二阶段；因权重为非商业许可，基准输出只能保存在本地授权目录。
...
追加扫描记录在 `.dev/anima-parameter-sweep/report.json` 和 `.dev/anima-parameter-sweep/low-cfg-report.json`。2.9B 的 `denoise=0.35~0.45、CFG=3~4` 饱和色覆盖最高约 `0.75%`，暗线保留约 `66%~72%`；`denoise=0.25~0.35、CFG=1.5~2` 虽将暗线保留提高到约 `78.5%`，但饱和色覆盖为 `0%`。Base 的 LLLite 强度 `0.8~1.4` 饱和色覆盖约 `31%~60%`，暗线保留低于 `1%`。扫描未找到满足两项质量要求的参数窗口，正式档位保持原参数。
