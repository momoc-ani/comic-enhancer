...
真实基准中，MangaNinja 使用 GhostMix V2、25 步、节点线稿预处理和 PointNet 对应点，单格热推理约 8 秒、冷切换约 20 秒，因此作为可预推理的独立实验档，不满足快速模式秒级目标。4090 上并存其他 GPU 服务时必须预留采样峰值；OOM 会由 API 回退到 8190，不能通过降低匹配安全阈值或强制占用其他业务显存解决。
