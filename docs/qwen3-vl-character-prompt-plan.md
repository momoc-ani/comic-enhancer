...
推荐新增独立的 `flux2_character` 处理档位，并增加独立的 `Qwen3-VL-4B-Instruct` 视觉分析服务。视觉模型负责从漫画原图和有序角色参考图中识别角色、定位角色实例、提取有证据的颜色特征，并描述背景场景；现有 Qwen3 4B 文本编码器继续负责把整理后的提示词编码给 FLUX.2。两者职责不能混为一体。

该档位使用新的完整 ComfyUI 工作流：在同一次 FLUX.2 去噪中，用全局场景 conditioning 覆盖整页和背景，用每个角色自己的 conditioning 与参考图约束其区域，同时继续使用原图 latent、文字和墨线保护。这样比“先生成背景、再分别生成角色并拼接”更容易保持统一光照、色调和边缘连续性。
...
- 任一分析或推理阶段失败时，新档位直接返回失败，由插件继续显示原图；不得切换到其他模型档位。
...
建议优先使用 Instruct 版本而不是让视觉模型直接输出长篇自由文本，原因是本项目需要稳定的 JSON 契约、低温确定性输出和明确的失败语义。官方 Qwen3-VL 仓库提供多图输入、图片编号以及 boxes/points 定位能力，并推荐使用 vLLM 或 SGLang 部署。
...
Chrome 扩展
  -> Comic Enhancer API
       |- Flux2CharacterModeStrategy（独立档位策略）
       |- 元数据与角色参考服务
...
插件仍只访问 Comic Enhancer API，不感知 Qwen3-VL 或 ComfyUI。API 仍只配置一个 ComfyUI 地址；Qwen3-VL 是视觉分析能力，不是第二个生成后端。`CharacterVisionAnalyzer` 应定义为窄接口，便于替换 4B/2B、本机/独立内网部署，而不让模型 SDK 侵入任务编排和工作流加载器。`Flux2CharacterModeStrategy` 独立拥有可用性、缓存版本、角色参考策略和处理逻辑，不复用现有 `Flux2ModeStrategy` 的业务处理函数。
...
8. API 校正输出几何，记录实际分析版本、绑定计划、工作流版本和缓存元数据，再返回不可变 WebP。
...
| 契约 | 建议值 |
| --- | --- |
| API mode | `flux2_character` |
| 策略类 | `Flux2CharacterModeStrategy` |
| 成功结果 `model_profile` | `flux2-klein-4b-qwen3-vl-character` |
| 工作流文件 | `workflows/flux2-klein-4b-qwen3-vl-character-colorize.json` |
| 能力字段 | `flux2_character_available` |
| 扩展显示名称 | `角色稳定模式（Qwen3-VL + FLUX.2）` |

档位可用性必须同时满足：功能开关开启、独立工作流通过加载器校验、FLUX.2 相关模型和节点就绪、Qwen3-VL sidecar 健康、视觉模型版本匹配。任一条件不满足时，能力接口不声明该档位可用。

该策略不包含任何模型档位回退：

```text
Qwen3-VL 分析成功 + 页面计划有效 + 新工作流成功
  -> 返回 flux2-klein-4b-qwen3-vl-character

Qwen3-VL 超时/不可用、JSON 无效、页面计划无效、工作流失败
  -> 本档位请求失败
  -> 插件保留原图
```

“过滤单个低置信度角色实例”属于页面计划校验，不是档位回退。只要页面计划整体有效，工作流仍可用全局场景 conditioning 为背景和未绑定区域上色；如果计划不满足新档位定义的最低要求，则直接失败。成功响应不得报告现有 `flux2`、`flux2_quant` 或 `quality` 的 `model_profile`。
...
第一阶段建立独立档位契约和视觉分析链路，不改造现有 `flux2`：

- 增加 `ProcessingMode.FLUX2_CHARACTER`、`Flux2CharacterModeStrategy`、独立能力字段、配置项和缓存 revision。
- 扩展设置和模型状态增加新的用户可见档位；依赖未全部就绪时不对外声明可用。
- 增加独立的 `CharacterVisionAnalyzer` 窄接口及 Qwen3-VL sidecar 实现。
- 保留角色 ID、名称、摘要、参考图槽位和图像字节，不再把角色库立即降级为无语义的 `dict[str, bytes]` 值序列。
- Qwen3-VL 生成严格页面计划，服务端通过固定模板构造全局场景提示和角色提示。
- 增加独立的工作流加载、校验和绑定入口，不调用现有 `_run_flux2_prompt` 作为替代路径。
- 取消“重复最后一张参考图填满槽位”；新工作流应支持可选角色分支，未使用槽位不得参与 conditioning。
- Qwen3-VL 超时、JSON 无效、页面计划无效或服务不可用时，本档位直接失败并记录不含受保护内容的失败阶段。

可以用动态全局提示作为离线 A/B 对照，但它不是新档位的生产工作流。新档位只有在角色区域绑定链路完成后才声明可用。
...
该阶段会新增模型、依赖、显存和延迟，不建议在新档位首版直接引入。
...
新档位使用独立的 `FLUX2_CHARACTER_PROCESSING_REVISION`，不得与现有 `flux2` 或 `flux2_quant` 共用最终结果缓存。缓存键还必须加入：

- 视觉分析功能开关和分析版本；
- 实际角色档案摘要及槽位顺序；
- 页面角色实例区域或掩码摘要；
- 场景调色板版本；
- `flux2_character` 工作流缓存版本和成功执行的 `model_profile`。

失败请求不得写入成功结果缓存；可以单独记录短期失败状态用于限流，但不能把失败状态当作图片结果返回。
...
不能让 Qwen3-VL sidecar 和 ComfyUI 绕过统一 GPU 并发控制同时抢占显存。视觉分析超时应短于页面推理总超时；超时后终止本档位请求，不提交 ComfyUI，也不调用其他处理策略。
...
- 每页实际使用的角色档案、区域策略、分析 revision、工作流 revision、失败阶段和错误码。

低置信度角色实例必须过滤，不能为了提高角色覆盖率强行应用颜色；过滤后页面计划不再满足最低契约时，本档位直接失败。建议先用 30 至 50 页结果校准置信度、IoU 和串扰门槛，再由用户审核最终数值；未经 100 页门禁，新档位可以保持显式可选，但不应设为默认路线。

## 需要用户审核的决策

用户已确认该能力使用独立工作流、独立档位策略，并且不进行任何模型档位回退。实现前仍需要确认以下资源和质量决策：
...
这些决策和 A/B/C 基准结果不影响新档位独立性的约束。在完成审核前，不修改现有档位工作流，也不把新档位设为默认路线。
