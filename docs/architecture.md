...
拷贝漫画页面
  ...
  -> 调度器优先当前可视页，再按页码预生成当前话和可配置数量的后续章节
  -> 插件后台下载原图
...
浏览器原生 Web Crypto 解密该列表，不执行远端页面脚本。当前话预生成任务按页码串行上传；后续章节
由适配器逐话解析下一话链接，最多按插件设置上传 0–20 话。插件只负责使用当前浏览器 Cookie 下载并上传
原图，服务端收到 `202/job_id` 后独立处理，浏览器关闭或重启不会丢失已上传任务。当前话未生成页面优先于
后续章节，后续章节优先级按距离递增；循环链接、跨域跳转和重复章节会停止发现。下一话任务只生成服务端
缓存，不提前下载增强结果，用户进入下一话后再按视口优先级读取缓存并覆盖显示。

拷贝漫画及 `copy3000.com` 镜像统一使用 `copy_manga` 来源身份。插件只在
`/comic/{作品}/chapter/{章节}` 阅读页启动处理；首页、排行榜、搜索和作品详情页不注入推理逻辑。
阅读页可同源读取作品详情元数据以补全规范标题和别名，但不会处理详情页图片。
...
插件与 API 之间的图片传输由独立传输层负责：插件把 multipart 请求序列化后使用浏览器原生 `CompressionStream("gzip")` 压缩，并在 gzip 无收益或浏览器不支持时回退普通 multipart；服务端在 FastAPI 路由前透明解压并限制压缩体和解压体大小。API 的 JSON 响应可使用 gzip；增强结果和章节原图缓存的图片响应只有在 gzip 后确实更小时才压缩，否则保持原始图片字节和文件响应路径。压缩只改变 HTTP 传输表示，不重新编码图片；服务端解压后的图片字节与插件上传字节完全一致，浏览器收到响应后自动还原原始响应字节。
...
- 角色线稿保真档独立管理最多 3 张角色参考图，使用同样的 `0.85MP` 四步空 latent 工作流；ComfyUI 恢复原图尺寸后，服务端保留原图高频线稿/网点结构，并以生成图的低频明度和色度补充背景与角色层次，再交给 UPSCALE 策略执行一次 Real-CUGAN 2x。该档位强制跳过角色档直出开关，不改变其他策略。
...
`flux2_character_lineart` 是独立的线稿保真档位。它复用 Qwen3-VL 角色库和三张参考图，但工作流、策略、缓存版本和模型标识均独立；服务端固定保留原图高频线稿/网点，并融合 FLUX.2 的低频明度与色度，禁止 `ComfyUI 原图直出`。首版不引入页面分格检测，先验证整页高密度线稿的结构稳定性。

三个 FLUX.2 正式档位使用独立提示词职责：`flux2` 优先完整场景色彩和层次，参考图只提供通用调色依据；`flux2_character` 在原图结构之后优先角色身份和跨分格配色一致性，并通过 `REFERENCE_IMAGE_n` 到角色名的静态映射限制调色板传播；`flux2_character_lineart` 优先原图线稿、网点、文字和几何，只要求 FLUX.2 生成与已有区域对齐的低频明度和色度。三者都禁止参考图带入人物、姿势、服装结构或背景，提示词变化分别通过工作流摘要或 `PROMPT_PLANNER_REVISION` 进入缓存键。
...
当前另提供两个默认关闭、彼此独立的验收档位。`flux2_9b_lora` 使用 FLUX.2 Klein 9B FP8、Qwen3 8B FP8 mixed、9B 一致性 LoRA 0.6、三张角色参考图和四步空 latent，作为画质候选；`flux2_4b_source` 使用 FLUX.2 Klein 4B FP8、4B 一致性 LoRA 0.35、三张角色参考图、四步 source latent、CFG 1.3 和 denoise 0.65，作为快速结构稳定候选。两档均由 ComfyUI 恢复为原图尺寸后直接输出，再进入现有 Real-CUGAN 2x 二阶段，不调用 Qwen3-VL 页面分析，也不执行额外的明度、文字或墨线回注。ControlNet 结构约束作为后续独立实验，不改变这两个基线档位。

`flux2_character` 使用独立的 FLUX.2 四步空 latent 工作流，以漫画原图和三张已建档角色参考图构造 `ReferenceLatent` 条件，不增加页面级 mask 分支。角色档案中的完整调色板仅作为 palette-only 正向提示补充；不确定性限制只约束角色颜色复制，不能让背景、建筑、家具和物体保持灰阶。没有角色图、Qwen3-VL 不可用或角色档案分析失败时，角色稳定与线稿保真档分别切换到独立单输入 FLUX.2 工作流，只使用漫画原图作为条件，不上传角色图或注入角色调色板，也不回退 SD1.5。执行方案在缓存版本计算时选定并复用于实际推理，避免一次请求重复分析及缓存工作流错配。普通路径下工作流输出恢复原图几何后以固定 `1.80x` 增益保留全页生成色度，只回注原图明度、文字和深色墨线；直出模式跳过这些代码处理，仍由外层本地 Real-CUGAN 执行二阶段放大。启用原图分辨率实验开关后，漫画输入按原图像素量生成，工作流内完成宽高校正，服务端只在非直出模式执行结构保护，Real-CUGAN 直接从接近原图尺寸的阶段结果进行 2x 放大。该增益、直出状态、原图分辨率状态和缓存版本只属于角色档，不影响其他策略。新档位的页面缓存键仍包含原图哈希，但角色上下文键只包含作品、角色参考图和模型版本，因此同一作品的角色档案可跨页复用。
`flux2_4b_color` 是默认关闭的第三个独立验收档，保留 `flux2_4b_source` 的 source latent、四步、CFG 1.3、denoise 0.65 和三参考图链路，只把一致性 LoRA 强度从 0.35 调整为 0.25。四页参数扫描中饱和色覆盖由 32.96% 提高到 36.34%，暗线保留约 95.28%，用于比较“颜色完成度优先”和“结构余量优先”；它不替换或修改原结构稳定档。

`flux2_9b_fast` 是默认关闭的独立速度验收档，模型、LoRA、提示词、参考图、采样和后处理均与 `flux2_9b_lora` 相同，仅通过专用工作流启用 `UNETLoader.weight_dtype=fp8_e4m3fn_fast`。该档拥有独立开关、工作流、策略、能力字段、缓存版本和模型标识，不能隐式替换 9B 画质档。

`flux2_9b_fast_lowres` 是建立在 `flux2_9b_fast` 上的默认关闭低分辨率实验档。它只把漫画页 `ImageScaleToTotalPixels` 从 `0.85MP` 改为 `0.70MP`，参考图仍为三张 `0.25MP`，其余推理和 Real-CUGAN 二阶段契约保持不变。该档使用独立能力字段、工作流、策略、缓存版本和模型标识；因四页冒烟只获得约 8.4% 内部热推理收益且存在暗线退化，不能自动替换任何现有档位。
...
角色库优先使用 `external_ids` 精确命中的数据源，再考虑标题搜索候选；只有置信度不低于 `0.6` 的候选才可进入参考库。标题包含明确的“第 N 章/篇/部/季/卷/幕/期”篇章标记时，元数据聚合器还会依次尝试篇章标题和母系列标题，例如从“Re：從零開始的異世界生活 第四章 聖域與強欲的魔女”派生“Re：從零開始的異世界生活 第四章”和“Re：從零開始的異世界生活”。
