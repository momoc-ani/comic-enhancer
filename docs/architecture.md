...
拷贝漫画页面
  ...
  -> 可视区域调度器优先当前页并预取后续页
  -> 插件后台下载原图
  -> 唯一漫画增强 API 选择快速/质量/MangaNinja/Cobra/FLUX.2 工作流及作品 LoRA / 通用 LoRA / 无 LoRA
  -> API 向唯一 ComfyUI 容器提交对应工作流的单并发 GPU 任务
  -> ComfyUI 结果返回 API 进行文字/墨线保护、缓存和鉴权
  -> 缓存不可变的 WebP 结果和推理元数据
  -> 插件后台鉴权取图并覆盖显示
  -> 任一步失败都保留原图
```

浏览器插件只访问漫画增强 API。快速、质量、MangaNinja、Cobra 和 FLUX.2 都是同一个 ComfyUI 容器中的预设工作流；后端只配置一个 `comfyui_url`，不能按档位配置第二个推理地址。MAGIv2 只在 API 内部为 MangaNinja 提供人物分析，不映射宿主机端口。
...
作品配置还可以登记跨站角色 ID 和名称别名。人物分析前会把同一角色的 Bangumi/AniList 图片合并，只保留尺寸可用且包含有效颜色的候选，再按已确认作品来源、有效色彩、完整人物构图、分辨率、饱和度、细节和来源优先级分别选出默认、头像和全身上色参考；纯色背景上纵向覆盖完整的角色立绘提供服装配色，头像参考提供近景发色、眼睛和肤色。MAGIv2 可同时使用同角色的多个站点图片进行身份匹配，候选距离按逻辑角色聚合，第二候选必须是另一个角色；MangaNinja 在身份已接受后按人物框相对分格的构图选择头像或全身彩色图。只有灰度图的角色暂不进入上色参考库。
...
MangaNinja 档先通过统一 ComfyUI 的作品 LoRA、通用 LoRA 或基础质量工作流生成整页底图，再只处理含可靠角色的分格，每格最多 4 个角色。每个可靠人物以人物框加受分格约束的上下文单独推理，参考图和目标人物使用头部、上身、腰部和下身 4 对纵向颜色锚点；极小人物的锚点发生坐标重叠时回退单中心点。该方式避免小人物在整分格中占比过低、多人物参考色互相串色、单点只传递占比最大的发色，以及只给已匹配角色着色而让全页退化成少数参考主色。生成后恢复聚焦区域几何，仅在对应 SAM 人物掩码内覆盖质量底图，并保留原图中文、网点和墨线；`processed_panels` 仍按实际受影响分格去重计数。参考链路失败时直接保留已生成的质量底图，不重复推理。该链路只能由独立实验档显式触发，未通过至少 100 页授权集前不替代普通质量档。
...
- 当前可视页优先级最高，之后按页面顺序处理预取队列；MangaNinja 以每 8 页为一个分析窗口，队列进入第 9、17 页等边界时先分析对应窗口再继续推理，避免 MAGIv2 与 MangaNinja 同时抢占 GPU，后续页也不会因为只分析章节首页而静默回退质量档。
