...
实验参考图选择顺序为：拷贝漫画封面 -> Bangumi 封面 -> AniList 封面 -> Kitsu 封面 -> Shikimori 封面 -> MAL 封面。只有置信度不低于 `0.6` 的外部匹配才会作为参考图；角色图片和角色介绍首版仅用于角色档案、调色板候选和后续作品级 LoRA 训练，不会把一组未确认角色图直接输入 MangaNinja。由于整页 MangaNinja 未通过三张多格中文页验收，默认配置不启用参考工作流路由。
