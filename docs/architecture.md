...
- FLUX.2 独立管理最多 3 张参考图、切换时释放 Cobra，并在自己的 ComfyUI 工作流内完成源图 latent 结构约束、窄文字/墨线保护和无生成式 2 倍缩放；API 不复用 SD1.5 的后端业务后处理。
...
```text
原始漫画页
  -> 原图编码为 FLUX.2 采样 latent，不再从空 latent 重画整页
  -> 0.85MP FLUX.2 八步调度，以 0.85 去噪强度完成上色
  -> 用阈值 0.18 的窄深色像素蒙版覆盖原始文字和关键墨线
  -> Lanczos 直接输出工作流 2x 结果，不再运行会制造网点伪纹理的神经超分
  -> API 按原图比例居中裁剪，并精确输出原图宽高各 2x
```

工作流禁止使用空 latent 重新生成整页，也禁止使用原图明度对整页做 Color 混合。提示词同时把字形、标点、数字、换行和空白声明为不可变源像素，原图蒙版只覆盖最深的文字核心与关键墨线。Anime 6B 和通用 Real-ESRGAN 在三页基准中都会把漫画网点强化为伪文字状纹理，因此不进入 FLUX.2 正式输出链路。
