...
- FLUX.2 独立管理最多 3 张参考图、切换时释放 Cobra，并在自己的 ComfyUI 工作流内完成文字/墨线保护和 Anime 6B 最终超分；API 不复用 SD1.5 的后端业务后处理。
...
原始漫画页
  -> 0.85MP FLUX.2 四步上色
  -> 以原图明度恢复结构，并用深色像素蒙版覆盖文字和墨线
  -> Real-ESRGAN Anime 6B 对最终上色结果执行 4x 超分
  -> Lanczos 回缩为工作流 2x 输出
  -> API 按原图比例居中裁剪，并精确输出原图宽高各 2x
```

超分不进入 FLUX.2 输入端和扩散 latent，不扩大生成阶段的输入分辨率。提示词同时把字形、标点、数字、换行和空白声明为不可变源像素，但文字正确性以工作流的原图蒙版覆盖为最终保障。
