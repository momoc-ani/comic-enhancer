...
`flux2_character` 复用可直接提交的 FLUX.2 reference colorize 工作流，不增加页面级 mask 分支，也不做原图明度、文字或墨线回注。页面输入只作为生成源图；角色档案中的完整调色板作为正向提示词补充，工作流直出后再由外层本地放大。新档位的页面缓存键仍包含原图哈希，但角色上下文键只包含作品、角色参考图和模型版本，因此同一作品的角色档案可跨页复用。
