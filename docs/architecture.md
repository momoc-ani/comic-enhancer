...
MangaNinja 只处理含可靠角色的分格，每格最多 4 个角色。每个可靠人物以人物框加受分格约束的上下文单独推理，使用参考人物中心与目标人物中心的成对点约束，避免小人物在整分格中占比过低或多人物参考色互相串色。生成后恢复聚焦区域几何，仅在对应 SAM 人物掩码内回注颜色，并保留原图中文、网点和墨线；`processed_panels` 仍按实际受影响分格去重计数。无可靠参考或参考链路失败时，同一请求回退到 8190 的作品 LoRA、通用 LoRA 或基础质量工作流。该链路只能由独立实验档显式触发，未通过至少 100 页授权集前不替代普通质量档。
