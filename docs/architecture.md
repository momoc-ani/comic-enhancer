...
拷贝漫画页面
  ...
  -> 可视区域调度器优先当前页并预取后续页
  -> 插件后台下载原图
  -> API 选择快速/质量完整工作流及作品 LoRA / 通用 LoRA / 无 LoRA
...
角色库优先使用 `external_ids` 精确命中的数据源，再考虑标题搜索候选；只有置信度不低于 `0.6` 的候选才可进入角色库。质量模式先用 MAGIv2 对最多 8 页做分格、人物检测和跨页保守聚类，再以“最近角色距离 + 第二候选差距 + 跨页一致锚点”决定接受或拒绝。没有可靠身份的分格不会进入 MangaNinja，不能按角色列表顺序强制绑定。

MangaNinja 只处理含可靠角色的分格，每格最多 4 个角色，使用参考人物中心与目标人物中心的成对点约束。生成后恢复原分格几何，并回注原图明度、中文、网点和墨线。参考链路失败时同一请求回退到 8190 的作品 LoRA、通用 LoRA或基础质量工作流。该链路仍默认关闭，完成更多作品的质量基准后再开放浏览器默认路由。
