...
作品配置还可以登记跨站角色 ID 和名称别名。人物分析前会把同一角色的 Bangumi/AniList 图片合并，只保留尺寸可用且包含有效颜色的候选，再按已确认作品来源、有效色彩、完整人物构图、分辨率、饱和度、细节和来源优先级选出唯一上色参考；纯色背景上纵向覆盖完整的角色立绘优先于高饱和头像，因为前者能给头部、上身、腰部和下身锚点提供对应的服装配色。MAGIv2 可同时使用同角色的多个站点图片进行身份匹配，候选距离按逻辑角色聚合，第二候选必须是另一个角色；MangaNinja 始终使用择优后的彩色图。只有灰度图的角色暂不进入上色参考库。
