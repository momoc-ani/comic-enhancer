...
class GiteeAdapterStore:
    """Gitee-backed index and release asset store.

    The client intentionally exposes no delete operation. Publishing creates a
        ...
        with self._client() as client:
            ...
            with temporary.open("wb") as stream:
                if download_url:
                    ...
                    payload = response.json()
                    stream.write(base64.b64decode(payload["content"].replace("\n", "")))
    ...
    def _update_index(self, manifest: AdapterManifest, message: str) -> None:
        ...
        if manifest.work_key:
            ...
            raise GiteeError("发布清单必须包含 work_key，或匹配当前 generic adapter_id")
    ...
    def _validate_download_url(download_url: str) -> None:
        ...
        if parsed.scheme != "https" or not (host == "gitee.com" or host.endswith(".gitee.com")):
